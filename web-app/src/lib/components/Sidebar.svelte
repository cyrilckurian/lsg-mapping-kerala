        ...
        <div class="space-y-8">
          ...
          <section>
            <div class="flex items-center justify-between mb-4 px-1">
              <h3
                class="text-xs font-black uppercase tracking-[0.2em] text-slate-600"
              >
                Local Leadership
              </h3>
            ...
            <div class="grid gap-4">
              <!-- President/Mayor -->
              <div
                class="bg-gradient-to-br from-white/5 to-transparent rounded-2xl p-5 border border-white/5 hover:border-brand-primary/40 transition-all group relative overflow-hidden"
              >
                <div
                  class="absolute top-0 right-0 w-24 h-24 bg-brand-primary/5 rounded-full -mr-8 -mt-8 blur-2xl group-hover:bg-brand-primary/10 transition-colors"
                ></div>
                <div class="flex items-start gap-4 relative z-10">
                  <div
                    class="bg-brand-primary/10 p-2.5 rounded-xl group-hover:bg-brand-primary/20 transition-all group-hover:scale-110"
                  >
                    <User class="w-6 h-6 text-brand-secondary" />
                  </div>
                  <div class="flex-1">
                    <span
                      class="text-[10px] font-black text-slate-500 uppercase tracking-widest"
                      >President / Mayor</span
                    >
                    <h4
                      class="font-bold text-lg text-white group-hover:text-brand-secondary transition-colors"
                    >
                      {$selectedLSG.president_name || "Verification Pending"}
                    </h4>
                    <div class="flex flex-col gap-2 mt-3">
                      {#if $selectedLSG.president_contact}
                        <div
                          class="flex items-center gap-2 text-sm text-slate-400 font-medium hover:text-white transition-colors cursor-pointer"
                        >
                          <Phone class="w-3.5 h-3.5" />
                          <span>{$selectedLSG.president_contact}</span>
                        </div>
                      {/if}
                      {#if $selectedLSG.president_email}
                        <div
                          class="flex items-center gap-2 text-sm text-slate-400 font-medium hover:text-white transition-colors cursor-pointer"
                        >
                          <Mail class="w-3.5 h-3.5" />
                          <span>{$selectedLSG.president_email}</span>
                        </div>
                      {/if}
                    </div>
                  </div>
              ...
              >
                ...
                <div class="flex items-start gap-4 relative z-10">
                  <div
                    class="bg-brand-primary/10 p-2.5 rounded-xl group-hover:bg-brand-primary/20 transition-all group-hover:scale-110"
                  >
                    <Landmark class="w-6 h-6 text-brand-secondary" />
                  </div>
                  <div class="flex-1">
                    <span
                      class="text-[10px] font-black text-slate-500 uppercase tracking-widest"
                      >Secretary / Commissioner</span
                    >
                    <h4
                      class="font-bold text-lg text-white group-hover:text-brand-secondary transition-colors"
                    >
                      {$selectedLSG.secretary_name || "Verification Pending"}
                    </h4>
                    <div class="flex flex-col gap-2 mt-3">
                      {#if $selectedLSG.secretary_contact}
                        <div
                          class="flex items-center gap-2 text-sm text-slate-400 font-medium hover:text-white transition-colors cursor-pointer"
                        >
                          <Phone class="w-3.5 h-3.5" />
                          <span>{$selectedLSG.secretary_contact}</span>
                        </div>
                      {/if}
                    </div>
                  </div>
          ...
          <section>
            ...
            <div class="grid grid-cols-2 gap-3">
              {#if $selectedLSG.website}
                <a
                  href={$selectedLSG.website}
                  target="_blank"
                  class="flex flex-col items-center justify-center gap-1.5 bg-brand-primary/5 hover:bg-brand-primary/10 p-4 rounded-2xl border border-white/5 hover:border-brand-primary/30 transition-all group"
                >
                  <Globe
                    class="w-5 h-5 text-brand-secondary group-hover:scale-110 transition-transform"
                  />
                  <span
                    class="text-xs font-bold text-slate-400 group-hover:text-white"
                    >Portal</span
                  >
                </a>
              {:else}
                <div
                  class="flex flex-col items-center justify-center gap-1.5 bg-slate-800/20 p-4 rounded-2xl border border-white/5 opacity-50 grayscale"
                >
